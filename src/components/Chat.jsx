...
function Chat() {
  ...
  const [poopStatus, setPoopStatus] = useState({ lastGotPooped: 0 });
  ...
  async function getData() {
    const res = await supabase
      .from("messages")
      .select("*")
      .gt("timestamp", new Date().getTime() - oldestMessageTime);
  ...
  async function deleteOldMessages() {
    const { error } = await supabase
      .from("messages")
      .delete()
      .lt("timestamp", new Date().getTime() - oldestMessageTime);
  }
                  ...
                  <div id="chat" className="chat">
                        ...
                        <>
                          {messages.length > 0 ? (
                              ...
                              {messages.map((value) => {
                                return (
                                  <Message
                                    ...
                                    pfpUrl={value.pfpUrl}
                                    sentByEmail={value.sentByEmail}
                                    userLastPooped={poopStatus}
                                  />
                                );
                              })}
                            </>
                            ...
                            <div className="no-messages">
                              <p className="no-messages-text no-select">
                                No messages were sent in the past 48 hours.
                              </p>
                            </div>
                          )}
                        </>
                    ...
                    <div className="about-link-wrp">
                      <a href="/about" className="about-link">
                        About PoopChat <span className="arrow">→</span>
                      </a>
                    </div>
                    <form
                      onSubmit={(e) => {
                        ...
                        setRandomPlaceholder(
                          inputPlaceholders[
                            Math.floor(Math.random() * inputPlaceholders.length)
                          ]
                        );
                        setMessage("");
                      }}
                    ...
                    >
                      ...
                      >
                        <div className="input-pfp-btn">
                          <img
                            className="input-pfp-img no-select"
